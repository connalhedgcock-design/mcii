...
| D-99 | social collection is TWO TRACKS over ONE purchase: a strict `emerging` signal (names a coin + account looks human + not an advert, all three) and a bulk fact record of every post paid for. ∵ cost is per post FETCHED — filtering afterwards is free, so two pipelines would be strictly worse than one wide buy with two treatments, and both tracks then describe the same posts and can be cross-referenced. 874 previously-fetched posts had been dropped entirely; nothing is discarded now. | 09-01 | — |
| D-100 | ! discovery counts are WEIGHTED by account credibility, never head-counted. three week-old accounts posting 400x/day weigh 0.0, not 3.0. raw AND weighted both reported — the gap between them is itself the signal (12 accounts weighing 1.4 is a campaign, one number would hide it). `shared/credibility.js`, grounded in the bot-detection literature per `60-KB/social-signal-research.md`. ! never content-based: scoring what an account SAYS collapses into "posts I agree with". | 09-01 | — |
| D-101 | ! coordination = same DISTINCTIVE wording from DIFFERENT accounts in a tight window. both halves required (timing alone = a busy hour; wording alone = a popular phrase). posts under 8 words never fingerprinted. ! rings feed BACK into discovery — without that a coordinated push yields the STRONGEST signal, since each sockpuppet passes the per-post check individually. flagged coins are MARKED, never deleted (D-69). | 09-01 | — |
