...
Steps:
1. Read `80-WHISPERS/INBOX.md` and every `w-*.md`. Assign `w-NNN` ids to any uncaptured inbox lines
   (zero-padded, never reused, in the order they appear).
...
- Prefer whisper↔whisper links over whisper→project. Two independent thoughts converging says
  something about how he actually thinks; a whisper matching a project you already know about is
  the easiest and least informative link available.
